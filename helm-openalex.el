;;; helm-openalex.el --- Org-ref interface to OpenAlex reviewed for Helm 
...
(defun oa--author-entries (works-data url &optional slice)
  ...
  (let* ((meta (plist-get works-data :meta)) 
    ...
    (message "Count: %s, Pages: %s, Per-Page: %s\n" count pages per-page)
  ...
  (or
   ...
   (let* ((url "https://api.openalex.org/autocomplete/authors")
     ...
     (cl-loop for author in results collect
	      (let* ((author-oaid (plist-get author :id))
		     (hint (plist-get author :hint))
		     (count (plist-get author :works_count))
		     )
		(concat
		 (propertize
		  (format "%-20s - %-32s \t"
			  (s-truncate 20 (plist-get author :display_name))
			  (s-truncate 32 (if hint hint " "))
			  )
		  'work-count count
		  'oaid author-oaid)
		(propertize
		 (format " [Work Count: %5s]" count)
		 'font-lock-face '(:foreground "sky blue"))
		)
		)))))
...
(defun oa--author-org (entity-id &optional slice)
...
"
      ...
      (insert (s-join "\n" (oa--author-entries works-data works-url slice)))
...
(provide 'helm-openalex)

;;; helm-openalex.el ends here
